...
% This function has four required inputs and two optional ones:
...
%   5 (optional). include_nonlinear: a flag which, when set to 1, includes
...
%   6 (optional). is_demo: a flag which, when set to 1, signals that the
...
%       "amplitude," "acrophase," "cosinor_p," which pertain to cosinor fits
%       for visualization purposes. Additionally, new fields used for statistics
%       include "entropy," "cosinor_R2," "cosinor_matrix," "linearAR_R2," 
%       "linearAR_matrix," "nonlinearAR_R2," "nonlinearAR_matrix, and "kfold."

function percept_data = calc_circadian(percept_data,zone_index,cosinor_window_left,cosinor_window_right,include_nonlinear,is_demo)
...
for j = 1:size(percept_data.LFP_norm_matrix,1)
    ...
    for hemisphere=1:2
        ...
        for i = 1:length(days) %Iterating on hard-coded 1 day window in the dataset for sample entropy
            disp([percept_data.days{j,1},' - ',num2str(i)])
            if any((start_index > i & start_index <= i) | length(days) < i)
                % Skipping calculations if there are full-day or greater gaps in data in the specified window
            else
                y_filled = reshape(LFP_filled(:,i),[1,144]);
...
for m = 1:length(models)
    for hemisphere = 1:2
        ...
        python_data.([models{m},num2str(hemisphere)]) = pyrunfile("calc_circadian_advanced.py","saveDict",hemi=py.int(hemisphere-1),mat_file=mat_file_path,...
            components=py.list(all_components),pt_id=py.list(cellfun(@(x) py.int(x),num2cell(0:size(percept_data.LFP_filled_matrix(:,1))-1),UniformOutput=false)),...
            pt_name=py.list(percept_data.LFP_filled_matrix(:,1)'),models=py.list(models(m)));
    end
end

percept_data = python_import_R2(python_data,percept_data,models); %Import data into matlab struct
